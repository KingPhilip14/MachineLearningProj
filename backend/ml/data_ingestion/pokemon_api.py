...
class PokemonApi(BaseApi):
    ...
    async def process_forms(self, session: aiohttp.ClientSession, default_form_name: str,
        ...
        for data in [default_data] + [form for form in significant_form_data if form]:
            ...
            movepool: list[dict] = await self.__get_movepool(session, data['moves'])
    ...
    async def __get_significant_forms(self, session: aiohttp.ClientSession, form_name: str,
        ...
        - Change the Pokémon's stats, ability(ies), typing, or movepool
        :param session:
        :param form_name:
        ...
        # check if the stats, type(s), abilities match, and movepools match
        ...
        movepools_equal: bool = default_form_data['moves'] == form_data['moves']

        # if collecting date for national dex, only check for cosmetic forms
        if self.generation == -1 and stats_equal and typing_equals and abilities_equal and movepools_equal:
            print(f'Cannot add '
        ...
        if stats_equal and typing_equals and abilities_equal and movepools_equal:
    ...
    async def __get_movepool(self, session: aiohttp.ClientSession, moves: list[dict]) -> list[dict]:
        move_urls: list[str] = [move['move']['url'] for move in moves]
        move_data: list[dict] = await asyncio.gather(*[self.fetch_json(session, url) for url in move_urls])
    ...
    async def __get_abilities(self, session: aiohttp.ClientSession, abilities: list[dict]):
        ...
        ability_urls: list[str] = [ability['ability']['url'] for ability in abilities]
